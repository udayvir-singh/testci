...
local function insert_stack(win_2a)
  _G.assert((nil ~= win_2a), "Missing argument win* on fnl/tangerine/utils/window.fnl:14")
  return table.insert(win_stack, {win_2a, vim.api.nvim_win_get_config(win_2a)})
end
local function remove_stack(idx_2a, conf_2a)
  _G.assert((nil ~= conf_2a), "Missing argument conf* on fnl/tangerine/utils/window.fnl:18")
  _G.assert((nil ~= idx_2a), "Missing argument idx* on fnl/tangerine/utils/window.fnl:18")
...
local function normalize_parent(win_2a)
  _G.assert((nil ~= win_2a), "Missing argument win* on fnl/tangerine/utils/window.fnl:26")
...
local function move_stack(start, steps)
  _G.assert((nil ~= steps), "Missing argument steps on fnl/tangerine/utils/window.fnl:52")
  _G.assert((nil ~= start), "Missing argument start on fnl/tangerine/utils/window.fnl:52")
  local index = start
...
local function nmap_21(buffer, lhs, rhs)
  _G.assert((nil ~= rhs), "Missing argument rhs on fnl/tangerine/utils/window.fnl:94")
  _G.assert((nil ~= lhs), "Missing argument lhs on fnl/tangerine/utils/window.fnl:94")
  _G.assert((nil ~= buffer), "Missing argument buffer on fnl/tangerine/utils/window.fnl:94")
  return vim.api.nvim_buf_set_keymap(buffer, "n", lhs, ("<cmd>" .. rhs .. "<CR>"), {silent = true, noremap = true})
end
local function setup_mappings(buffer)
  _G.assert((nil ~= buffer), "Missing argument buffer on fnl/tangerine/utils/window.fnl:97")
...
win["create-float"] = function(nlines, filetype, highlight)
  _G.assert((nil ~= highlight), "Missing argument highlight on fnl/tangerine/utils/window.fnl:104")
  _G.assert((nil ~= filetype), "Missing argument filetype on fnl/tangerine/utils/window.fnl:104")
  _G.assert((nil ~= nlines), "Missing argument nlines on fnl/tangerine/utils/window.fnl:104")
  ...
  local height = math.max(1, math.floor(math.min((win_height / 1.5), nlines)))
...
win["set-float"] = function(lines, filetype, highlight)
  _G.assert((nil ~= highlight), "Missing argument highlight on fnl/tangerine/utils/window.fnl:132")
  _G.assert((nil ~= filetype), "Missing argument filetype on fnl/tangerine/utils/window.fnl:132")
  _G.assert((nil ~= lines), "Missing argument lines on fnl/tangerine/utils/window.fnl:132")
