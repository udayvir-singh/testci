...
log.compiled = function(files, _3fverbose)
  _G.assert((nil ~= files), "Missing argument files on fnl/tangerine/utils/logger.fnl:28")
...
log.cleaned = function(files, _3fverbose)
  _G.assert((nil ~= files), "Missing argument files on fnl/tangerine/utils/logger.fnl:47")
  if (disable_3f(_3fverbose) or (0 == #files)) then
    return
  else
  end
...
local function serialize(tbl)
  _G.assert((nil ~= tbl), "Missing argument tbl on fnl/tangerine/utils/logger.fnl:57")
